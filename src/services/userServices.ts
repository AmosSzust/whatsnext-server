...
export const register = async (req: Request) => {
    ...
    try {
        await mailing(email, `Thank you for signing up, here's your confirmation code: ${code}`);
...
export const login = async (req: Request) => {
    ...
    if (!bcrypt.compareSync(password, user.password)) return [401, { error:  "Invalid email or password" }];
...
export const getSimilar = async (req: Request, res: Response) => {
    ...
    if (!Number.isInteger(yearsDifference) || yearsDifference < 0) return [400, { error: "Invalid value for difference in years" }];
    if (!Number.isInteger(numberOfEvents) || numberOfEvents < 1) return [400, { error: "Invalid value for number of events" }];
    if (!Number.isInteger(resultId) || resultId < 0) return [400, { error: "Invalid value for result id" }];
