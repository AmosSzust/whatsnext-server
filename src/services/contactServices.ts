...
export const addContact = async (req: Request, res: Response) => {
    ...
    if (!Number.isInteger(userLifeEventId)) return [400, { error: "An invalid identifier was sent" }];
    if (!Number.isInteger(sharedEventsCount)) return [400, { error: "An invalid events count was sent" }];
