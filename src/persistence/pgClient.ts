...
class PGClient {
    ...
    emailExists = async (email: string): Promise<boolean> => {
        const queryResult: QueryResult = await this.pool.query(`select id from users where email=$1`, [email]);
        return queryResult.rows.length > 0;
    };

    handleError = async (appError: AppError): Promise<void> => {
        const query: string = "insert into errors (error_when, error_message, callstack) values (now(),$1,$2)";
        try {
    ...
    getUser = async (email: string): Promise<IUserDB> => {
        const queryResult: QueryResult = await this.pool.query(`select * from users where email=$1`, [email]);
    ...
    getSimilar = async (
        ...
        if (resultId === 0) {
            ...
            selectResult = await this.pool.query(query, [email, yearsDifference, yearsDifference, numberOfEvents]);
        ...
        selectResult = await this.pool.query(query, [email, resultId, yearsDifference, yearsDifference, numberOfEvents]);
        return [selectResult.rows, listIds];
    };

    //Events
    getAllEvents = async (): Promise<ILifeEvent[]> => {
        const query: string = "select id, event_name, description, icon_name from life_events order by event_name asc";
        const queryResult: QueryResult = await this.pool.query(query);
        return queryResult.rows;
    };

    addEventToUser = async (eventToAdd: IEvent, email: string) => {
        const query: string = "insert into users_life_events (user_id, event_id, description, event_when) select id, $1, $2, $3 from users where email = $4";
        await this.pool.query(query, [eventToAdd.id, eventToAdd.description, eventToAdd.event_when, email]);
    };
